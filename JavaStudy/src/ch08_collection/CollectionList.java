...
public class CollectionList {
	...
	public static void main(String[] args) {
		...
		// 2. 오른쪽의 제너릭은 생략이 가능하다.
		ArrayList<String> strList = new ArrayList<>();
		
		// Collection 들은 객체만 담을 수 있다.
		// 객체 = 참조타입들
//		ArrayList<int> intList = new ArrayList<>();   // 참조타입이 아니라서 불가
		
		// Wrapper Class
		// 기본 타입의 데이터를 객체 (참조타입)로 사용하고 싶을 때 사용
		ArrayList<Integer> intList = new ArrayList<>();
		int num = 12;
		//	 기본 타입의 데이터를 Wrapper Class 객체로 변환한다.
		// = 박싱(boxing)
		...
		// 우리가 Wrapper클래스를 사용하는데에 불편함은 없다.
		...
		// syso에 컬렉션 객체를 넣으면, 자동으로 내부 값들이 출력된다.
...
//		students.add(12);
		...
		students.remove("백민기");
		System.out.println(students);
		students.remove("아무말");  // 해당 값이 없으면 아무 일도 일어나지 않음.
		System.out.println(students);
		...
		// 인덱스가 존재하는 ArrayList에서는 향상된 for문의 사용이 큰 메리트가 있지는 않음.
		...
		for(int k = 0; k < numbers.size()-1 ; k++) {
			for(int i = 0 ; i < numbers.size()-k-1; i++) {
				if(numbers.get(i) >= numbers.get(i+1)) {     // 오름차순
					int temp1 = numbers.get(i);
					numbers.set(i,numbers.get(i+1));
					numbers.set(i+1, temp1);
				}
			}
		}
		System.out.println(numbers);
